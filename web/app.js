...
async function post(path, payload) { const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 50000); try { const response = await fetch(path, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload), signal:controller.signal}); const body = await response.json(); if (!response.ok) throw new Error(body.error || 'Error.'); return body; } finally { clearTimeout(timeout); } }
async function api(method, path, payload) { const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 50000); try { const opts = {method, headers:{'Content-Type':'application/json'}, signal:controller.signal}; if (payload) opts.body = JSON.stringify(payload); const response = await fetch(path, opts); const body = await response.json(); if (!response.ok) throw new Error(body.error || 'Error.'); return body; } finally { clearTimeout(timeout); } }
...
function modeChanged() { const live = $('mode').value === 'live'; $('consent-wrap').hidden = !live; $('input').readOnly = !live; $('mode-note').textContent = live ? (config.live_available ? 'Gemini configurado.' : 'Gemini no configurado. Usa --ask-key.') : 'Ensayo con datos preparados.'; }
